...
private slots:
    ...
    void testSymbol()
    {
        ...
        QStringList actualTextList = QStringList();
        for (int row = 0; row < disassemblyOutput.disassemblyLines.size(); row++) {
            ...
            actualTextList << QString::number(disassemblyLine.addr) << disassemblyLine.disassembly
                           << QLatin1String("\n");
        }
        QString actualText = actualTextList.join(" ");
        QVector<DisassemblyOutput::DisassemblyLine> disassemblyLines = disassemblyOutput.disassemblyLines;
        {
