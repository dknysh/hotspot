...
QString SettingsDialog::sysroot()
{
    return ui->lineEditSysroot->text();
}
QString SettingsDialog::appPath()
{
    return ui->lineEditApplicationPath->text();
}
QString SettingsDialog::extraLibPaths()
{
    return ui->lineEditExtraLibraryPaths->text();
}

QString SettingsDialog::debugPaths()
{
    return ui->lineEditDebugPaths->text();
}

QString SettingsDialog::kallsyms()
{
    return ui->lineEditKallsyms->text();
}

QString SettingsDialog::arch()
{
    QString sArch = ui->comboBoxArchitecture->currentText();
    return (sArch == QLatin1String("auto-detect")) ? QLatin1String("") : sArch;
}

QString SettingsDialog::objdump()
{
